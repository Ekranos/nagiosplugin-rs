	...
	pub fn to_nagios_string(&self) -> String {
		...
		for checkable in self.checkables.iter() {
			...
			if let (Some(state), Some(description)) = (state, description) {
				if state != State::Ok {
					s.push_str(&format!("\n'{}' is {}: {}", checkable.name(), state.to_string(), description));
				}
			} else if let Some(state) = state {
				if state != State::Ok {
					s.push_str(&format!("\n'{}' is {}", checkable.name(), state.to_string()));
				}
...
impl StaticCheckable {
	pub fn new<S: ToString>(name: S, state: State, description: Option<S>) -> Self {
		Self { name: name.to_string(), description: description.map(|s| s.to_string()), state }
	}
}
...
#[cfg(test)]
mod tests {
	use crate::{Metric, PartialOrdMetric, Resource, SimpleMetric, State, ToPerfString, Unit, StaticCheckable};
